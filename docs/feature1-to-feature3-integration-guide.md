...
## 重要: 必ず6タイプのいずれかに分類してください

機能3の食材レコメンドは **6タイプのキーに完全一致** でデータを出し分けています。
`hairPattern` が渡されない、または6タイプ以外の文字列が渡された場合、パターン別の食材は表示されず、一般的なフォールバック表示になります。

**つまり、AI診断の結果は必ず以下の6つのいずれかに分類する必要があります。**
...
判断に迷う場合は、最も近いタイプを選んでください。「分類不能」は避けてください。
